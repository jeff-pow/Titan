...
fn negamax<const IS_PV: bool>(
    ...
    let corrected_eval;
    let estimated_eval;
    // With the addition of correction history, we have three values.
    // - raw_eval is the evaluation the network spits out with slightly scaled values based on (as
    // of the time of writing this comment) piece count and nearness to a 50 move draw. We always
    // want to store raw_eval in the tt
    // - corrected_eval is the value after raw_eval has been sent through the correction history
    // table. We need this to have an unaltered value to edit the correction history table after
    // the search
    // - estimated_eval is our best guess of the score of this position. It uses the search score
    // stored in the transposition table if there is one. If there is not a search score in a valid
    // tt entry, it uses the value in corrected_eval
    //
    if board.in_check() || singular_search {
    ...
    let improving = {
        if in_check {
            false
        } else if td.ply > 1 && td.stack[td.ply - 2].estimated_eval != -INFINITY {
            estimated_eval > td.stack[td.ply - 2].estimated_eval
        } else if td.ply > 3 && td.stack[td.ply - 4].estimated_eval != -INFINITY {
            estimated_eval > td.stack[td.ply - 4].estimated_eval
        } else {
