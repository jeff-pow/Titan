...
pub fn get_rook_moves(square: usize, blockers: u64) -> u64 {
    ...
    unsafe { ROOK_MOVES[magic_index(magic, blockers)] }
}

pub fn get_bishop_moves(square: usize, blockers: u64) -> u64 {
    let magic = &BISHOP_MAGICS[square];
    unsafe { BISHOP_MOVES[magic_index(magic, blockers)] }
}
...
unsafe fn gen_magic_board(table_size: usize, deltas: &[Direction; 4], attacks: *mut u64) {
    ...
    for s in 0..64u8 {
        ...
        // Find magics goes here
        pre_sq_table[s as usize].magic = if deltas[0] == Direction::North {
            ROOK_MAGICS[s as usize].magic
        } else {
            BISHOP_MAGICS[s as usize].magic
        };
    ...
    debug_assert_eq!(size, table_size);
}
...
fn sliding_attack(deltas: &[Direction; 4], square: u8, occupied: u64) -> u64 {
    ...
    for dir in deltas.iter() {
        ...
        while s < 64 && distance(square, square - *dir as u8) == 1 {
